...
def clean(donors, entity_ids):
    for id in entity_ids:
        ...
        for category in donors[index][str_id]:
            ...
            if (category == 'Honor') and (donors[index][str_id][category] != 0.0) and (isinstance(donors[index][str_id][category], list)):
                donors[index][str_id][category] = float(len(donors[index][str_id][category]))

            # current year (2018) minus class year
            ...
            # CALCULATE SUMS OF GIFT CATEGORIES
            if (category == 'Total Commitment') and (donors[index][str_id][category] != 0.0) and (isinstance(donors[index][str_id][category], list)):
            ...
            if (category == 'Hard Credit') and (donors[index][str_id][category] != 0.0) and (isinstance(donors[index][str_id][category], list)):
                if len(donors[index][str_id][category]) > 1:
                    total = 0.0
                    for i in donors[index][str_id][category]:
                        total += i
            ...
            # encode total commitment (our predictor) by bin for classifier
            if (category == 'Total Commitment Bin') and (donors[index][str_id][category] != 0.0) and not (isinstance(donors[index][str_id][category], float)):
                for i in donors[index][str_id][category]:
