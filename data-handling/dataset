...
loc = "/Users/benlehrburger/Desktop/Linear Regression Table.xls"
...
for row in entity_row:
    if row != 0:
        donors.append(Donor(row, entity, honors, gifts, honors_ids, gift_ids, entity_cats, honors_cats, gift_cats, categories).makeDonor())
        print('Donor ' + str(row))
...
for id in entity_ids:
    if entity_ids.index(id) < training_samples:
        ...
        for category in categories[:-1]:
            train_data.write(entity_ids.index(id)+1, categories.index(category)+1, donors[entity_ids.index(id)][str(id)][category])
        ...
        for category in categories[:-1]:
            test_data.write(entity_ids.index(id)+1-training_samples, categories.index(category)+1, donors[entity_ids.index(id)][str(id)][category])
        test_labels.write(entity_ids.index(id)+1-training_samples, 1, donors[entity_ids.index(id)][str(id)]['Total Commitment Bin'])

wb.save('test-dataset.xls')
